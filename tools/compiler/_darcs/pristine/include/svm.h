...
/* stack pointer */
#define svm_sp(name) (g_svm.stack_ ## name)
...
#define svm_push_obj(obj) do {					\
		if (svm_sp(top) == svm_sp(start))		\
			return;					\
		*(--svm_sp(stack_top)) = (Scheme_Obj_t *)(obj);	\
	} while (0)
...
#define NEW_ENVT(nslots) do {						\
		NEW_OBJECT(Envt);					\
		g_svm.reg_argc = sizeof(Scheme_Obj_t *)	* nslots * 2;	\
		alloc_c_obj();						\
		SCHEME_ENVT_BINDINGS(g_svm.reg_new) = g_svm.c_ptr;	\
	} while (0)
...
#define NEW_OBJECT(type) do {						\
		__asm__ volatile (					\
			...
			"addl $8, %%esp\n\t"				\
			:						\
			: "a"(Scheme_ ## type ## _Type),		\
			  "d"((sizeof(Scheme_ ## type ## _t)))		\
			: "memory");					\
	} while (0)
