...
addFunction name code = do
  ...
  cf <- getCFile
  envt <- getEnvt
  putCFile $ putFunctions ((functions cf) ++ [[fdecl label, "{"] ++ (body label envt) ++ ["}", ""]]) cf
  return label
    where
      fdecl fn = "static void " ++ "_c" ++ fn ++ "()"
      body fn envt = [funCallE "BEGIN_PROC" [fn], "",
                      funCallE "NEW_ENVT" [(show . length . getBindings) envt],
                      funCallE "EXTEND_ENVT" ["svm_register(new)"]] ++ code
...
instr2c (CallBuiltin (Sym fun) argc) = do
  -- addToHead [funCall "DEFINE_BUILTIN" [fun]]
  return $ [funCallBuiltin fun argc]
...
instr2c (MakeClosure (Sym name) (Args args) body) =
    do
      ...
      -- the enclosing of this closure will be initialize later in running code
      cl <- addConstData "Closure" (name ++ "_") ["NULL", tl]
      -- restore environment
      putEnvt env
